...
use App\Models\Tenant\MovimentosSaidas;
use App\Models\Tenant\MovimentosSaidasTemporary;
...
class SaidasDetail extends Component
{
    use WithPagination;
    use WithFileUploads;
    ...
    private ?object $saidaSpecific = null;
    ...
    public function mount($encomenda)
    {
        ...
        $this->saidaSpecific = $this->encomendaRepository->detalhesSaidas($this->encomenda, $this->perPage);

        $this->encomendaDocument = $this->encomendaRepository->entradaNumSaidas($this->encomenda);
    ...
    public function updatedCodbarras()
    {
        ...
        $encspe = $this->encomendaRepository->saidasconfcod($this->encomenda);
        ...
        if ($config->cod_barras_accept == 0 && $config->reference_accept == 1) {
            foreach ($encspe->lines as $line) {
                if ($this->codbarras == $line->referense) {
                    ...
                    $this->qtdstock = $this->encomendaRepository->saidasQtdStock($line->referense);
                }
        ...
        if ($this->codbarras == "") {
            ...
            if ($this->descricao == "") {
                // return to_route('tenant.saidasDetail', $this->encomenda)
    ...
    {
        $envguard = $this->encomendaRepository->saidasguardstock($this->encomenda);
        $array = [];
        ...
        if ($this->codbarras != "" && $this->descricao != "" && $this->qtd != "") {
            ...
            // Verifica se a quantidade em estoque é inferior à quantidade inserida
            foreach ($envguard->lines as $line) {
                if ($line->referense == $referenceGet) {
                    // Obtendo a quantidade de estoque para a referência específica
                    $this->qtdstock = $this->encomendaRepository->saidasQtdStock($line->referense);

                    // Verificando se a quantidade inserida é maior do que a quantidade disponível em estoque
                    if ($this->qtd > $this->qtdstock) {
                        // return to_route('tenant.saidasDetail', $this->encomenda)
                        //     ->with('message', 'A quantidade em Stock é inferior à quantidade adicionada!')
                        //     ->with('status', 'error');
                        $this->dispatchBrowserEvent('menssageerror', ['message' => 'A quantidade em Stock é inferior à quantidade adicionada!', 'status' => 'error']);
                    }
                }
            }

            $soma = 0;
            $checkbanc = MovimentosSaidasTemporary::where('numero_encomenda', $this->encomenda)->Where('referencia', $referenceGet)->get();
            ...
            if (empty($checkbanc)) {
                foreach ($envguard->lines as $line) {
                    if ($line->referense == $referenceGet) {
                        ...
                        MovimentosSaidasTemporary::create([
                        ...
                        // return to_route('tenant.saidasDetail', $this->encomenda)
                        //     ->with('message', 'Quantidade Separada foi alterada!')
                        //     ->with('status', 'sucess');
                ...
                foreach ($envguard->lines as $line) {
                    if ($line->referense == $referenceGet) {
                        ...
                        $sumQtdSeparada = MovimentosSaidas::where('numero_encomenda', $this->encomenda)
                            ->where('referencia', $referenceGet)
                            ->sum('qtd_separada');

                        $sumQtdtemporary = MovimentosSaidasTemporary::where('numero_encomenda', $this->encomenda)
                            ->where('referencia', $referenceGet)
                            ->sum('qtd_registrada');
                        ...
                        foreach ($envguard->lines as $line) {
                            if ($line->referense == $referenceGet) {
                                ...
                                // Verificando se a quantidade total separada ultrapassa a quantidade em estoque
                                if ($totalQtdSeparada > $line->quantity || $totalQtdSeparada > $this->qtdstock) {
                                    // return to_route('tenant.saidasDetail', $this->encomenda)
                                    //     ->with('message', 'Esta quantidade com as quantidades já adicionadas ultrapassam a Quantidade em Stock!')
                                    //     ->with('status', 'error');
                                    $this->dispatchBrowserEvent('menssageerror', ['message' => 'Esta quantidade com as quantidades já adicionadas ultrapassam a Quantidade em Stock!', 'status' => 'error']);
                                    return false;
                                }
                ...
                // return redirect()->route('tenant.saidasDetail', $this->encomenda)
                //     ->with('message', 'Quantidade Separada Adicionada!')
                //     ->with('status', 'success');
                $this->dispatchBrowserEvent('menssagesucess', ['message' => 'Quantidade Separada Adicionada!', 'status' => 'sucess']);
                return false;

            }
            // return to_route('tenant.saidadetail', $this->encomenda)
    ...
    {
        $mov_temp = MovimentosSaidasTemporary::where('numero_encomenda', $this->encomenda)->get();
    ...
    overflow-x: auto;'>";
        ...
        foreach ($referencias as $referencia => $dados) {
            $qtd_prevista = $dados['qtd_estoque'] - $dados['qtd_separada'];
    ...
    public function cancelarStock()
    {
        ...
        MovimentosSaidasTemporary::where('numero_encomenda', $this->encomenda)->delete();
    ...
    {
        $movimentosStockTemporarios = MovimentosSaidasTemporary::where('numero_encomenda', $this->encomenda)
        ...
        foreach ($movimentosStockTemporarios as $movimentoSaidas) {
            MovimentosSaidas::create([
                "numero_encomenda" => $movimentoSaidas->numero_encomenda,
                "id_line" => $movimentoSaidas->id_line,
                "document" => $movimentoSaidas->document,
                "referencia" => $movimentoSaidas->referencia,
                "barcode" => $movimentoSaidas->barcode,
                "descricao" => $movimentoSaidas->descricao,
                "locais_stock" => "",
                "estado" => "Saidas",
                "qtd_separada" => $movimentoSaidas->qtd_registrada,
                "internal_notes" => $movimentoSaidas->internal_notes,
                "stock" => $movimentoSaidas->stock,
                "warehouse" => $movimentoSaidas->warehouse,
            ]);
    ...
    public function enviar($encomenda)
    {
        ...
        $movimentos = DB::table('movimentos_saidas')
            ->where('numero_encomenda', $encomenda)
            ->get();

        // Verifica se há movimentos para enviar
        if ($movimentos->isEmpty()) {
            return redirect()->route('tenant.saidasDetail', $encomenda)
        ...
        foreach ($movimentos as $i => $movimento) {
            ...
            array_push($dadosMovimento["lines"], $dadosMovimentosaida);
        ...
        DB::table('movimentos_saidas')
            ->where('numero_encomenda', $encomenda)
            ->delete();

        return redirect()->route('tenant.saida', ['idsaida' => session('tipo_saida')])
    ...
    public function render()
    {
        ...
        return view('tenant.livewire.saidas.saidadetail', ["saidaDetail" => $this->saidaSpecific, "idSaida" => $this->encomenda]);
    }
}
