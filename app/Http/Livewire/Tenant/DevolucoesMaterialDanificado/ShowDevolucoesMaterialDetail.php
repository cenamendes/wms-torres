...
use App\Models\Tenant\MovimentosDevolucoesMaterial;
use App\Models\Tenant\MovimentosDevolucoesMaterialTemporary;
...
class ShowDevolucoesMaterialDetail extends Component
{
    use WithPagination;
    use WithFileUploads;
    ...
    private ?object $devmaterdanifSpecific = null;
    ...
    public function mount($encomenda)
    {
        ...
        $this->devmaterdanifSpecific = $this->encomendaRepository->detalhesdevolucoesmaterial($this->encomenda, $this->perPage);

        $this->encomendaDocument = $this->encomendaRepository->entradaNumDevolucoesDanificado($this->encomenda);
    ...
    public function updatedCodbarras()
    {
        ...
        $encspe = $this->encomendaRepository->devolumaterialconferecod($this->encomenda);
        ...
        if ($config->cod_barras_accept == 0 && $config->reference_accept == 1) {
            foreach ($encspe->lines as $line) {
                if ($this->codbarras == $line->referense) {
                    ...
                    $this->qtdstock = $this->encomendaRepository->devolmaterialQtdStock($line->referense);
                }
        ...
        if ($this->codbarras == "") {
            ...
            if ($this->descricao == "") {
                // return to_route('tenant.devolucoesmaterialdanificado.devolucoesmaterialdanificadodetail', $this->encomenda)
    ...
    {
        $envguard = $this->encomendaRepository->devolmaterialguardstock($this->encomenda);
        $array = [];

        if ($this->codbarras != "" && $this->descricao != "" && $this->qtd != "") {

            if ($this->qtd <= 0) {
                // return to_route('tenant.devolucoesclientes.devolucoesclientesdetail', $this->encomenda)
            ...
            $soma = 0;
            $checkbanc = MovimentosDevolucoesMaterialTemporary::where('numero_encomenda', $this->encomenda)->Where('referencia', $referenceGet)->get();
            ...
            if (empty($checkbanc)) {
                foreach ($envguard->lines as $line) {
                    if ($line->referense == $referenceGet) {
                        ...
                        MovimentosDevolucoesMaterialTemporary::create([
                ...
                foreach ($envguard->lines as $line) {
                    if ($line->referense == $referenceGet) {
                        ...
                        $sumQtdSeparada = MovimentosDevolucoesMaterial::where('numero_encomenda', $this->encomenda)
                            ->where('referencia', $referenceGet)
                            ->sum('qtd_separada');

                        $sumQtdtemporary = MovimentosDevolucoesMaterialTemporary::where('numero_encomenda', $this->encomenda)
                            ->where('referencia', $referenceGet)
                            ->sum('qtd_registrada');
                        ...
                        foreach ($envguard->lines as $line) {
                            if ($line->referense == $referenceGet) {
                                ...
                                if ($totalQtdSeparada > $line->quantity) {
                                    // return to_route('tenant.devolucoesclientes.devolucoesclientesdetail', $this->encomenda)
                                    //     ->with('message', 'Essa quantidade já ultrapassa a quantidade Separada!')
                                    //     ->with('status', 'error');
                ...
                // return redirect()->route('tenant.devolucoesclientes.devolucoesclientesdetail', $this->encomenda)
    ...
    {
        $mov_temp = MovimentosDevolucoesMaterialTemporary::where('numero_encomenda', $this->encomenda)->get();
    ...
    public function cancelarStock()
    {
        ...
        MovimentosDevolucoesMaterialTemporary::where('numero_encomenda', $this->encomenda)->delete();
    ...
    {
        $movimentosStockTemporarios = MovimentosDevolucoesMaterialTemporary::where('numero_encomenda', $this->encomenda)
            ->where('qtd_registrada', '!=', '0')
            ->get();

        if ($movimentosStockTemporarios->isEmpty()) {
            return redirect()->route('tenant.devolucoesmaterialdanificado.devolucoesmaterialdanificadodetail', $this->encomenda)
                ->with('message', 'Nenhum Dado encontrado para envio.')
                ->with('status', 'error');
        }

        foreach ($movimentosStockTemporarios as $movimentoStock) {
            MovimentosDevolucoesMaterial::create([
    ...
    public function enviar($encomenda)
    {
        ...
        $movimentos = DB::table('movimentos_devolucoes_material')
            ->where('numero_encomenda', $encomenda)
            ->get();

        // Verifica se há movimentos para enviar
        if ($movimentos->isEmpty()) {
            return redirect()->route('tenant.devolucoesmaterialdanificado.devolucoesmaterialdanificadodetail', $encomenda)
        ...
        foreach ($movimentos as $i => $movimento) {
            ...
            array_push($dadosMovimento["lines"], $dadosMovimentodev);
        ...
        DB::table('movimentos_devolucoes_material')
            ->where('numero_encomenda', $encomenda)
            ->delete();

        return redirect()->route('tenant.devolucoesmaterialdanificado', $encomenda)
    ...
    public function render()
    {
        ...
        //$this->devmaterdanifSpecific = $this->encomendaRepository->encomendaMovimentos($this->stringEncomenda,$this->perPage);

        $this->devmaterdanifSpecific = $this->encomendaRepository->detalhesdevolucoesmaterial($this->encomenda, $this->perPage);

        return view('tenant.livewire.devolucoesmaterialdanificado.devolucoesmaterialdanificadodetail', ["devolucoesmaterialdanificadodetail" => $this->devmaterdanifSpecific, "idEncomenda" => $this->encomenda]);
