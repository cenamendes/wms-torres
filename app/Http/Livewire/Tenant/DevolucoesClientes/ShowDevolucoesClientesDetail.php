...
use App\Models\Tenant\MovimentosDevolucoesClientes;
use App\Models\Tenant\MovimentosDevolucoesClientesTemporary;
...
class ShowDevolucoesClientesDetail extends Component
{
    use WithPagination;
    use WithFileUploads;
    ...
    private ?object $devolclientSpecific = null;
    ...
    public function mount($encomenda)
    {
        ...
        $this->devolclientSpecific = $this->encomendaRepository->detalhesdevolucoesclientes($this->encomenda, $this->perPage);

        $this->encomendaDocument = $this->encomendaRepository->entradaNumDevolucoesClientes($this->encomenda);
    ...
    public function updatedCodbarras()
    {
        ...
        $encspe = $this->encomendaRepository->devoluclientconferecod($this->encomenda);
        ...
        if ($config->cod_barras_accept == 0 && $config->reference_accept == 1) {
            foreach ($encspe->lines as $line) {
                if ($this->codbarras == $line->referense) {
                    ...
                    $this->qtdstock = $this->encomendaRepository->devolclientQtdStock($line->referense);
                }
    ...
    {
        $envguard = $this->encomendaRepository->devolclientguardstock($this->encomenda);
        $array = [];
        ...
        if ($this->codbarras != "" && $this->descricao != "" && $this->qtd != "") {
            ...
            $soma = 0;
            $checkbanc = MovimentosDevolucoesClientesTemporary::where('numero_encomenda', $this->encomenda)->Where('referencia', $referenceGet)->get();
            ...
            if (empty($checkbanc)) {
                foreach ($envguard->lines as $line) {
                    if ($line->referense == $referenceGet) {
                        ...
                        MovimentosDevolucoesClientesTemporary::create([
                ...
                foreach ($envguard->lines as $line) {
                    if ($line->referense == $referenceGet) {
                        ...
                        $sumQtdSeparada = MovimentosDevolucoesClientes::where('numero_encomenda', $this->encomenda)
                            ->where('referencia', $referenceGet)
                            ->sum('qtd_separada');

                        $sumQtdtemporary = MovimentosDevolucoesClientesTemporary::where('numero_encomenda', $this->encomenda)
                            ->where('referencia', $referenceGet)
                            ->sum('qtd_registrada');
    ...
    {
        $mov_temp = MovimentosDevolucoesClientesTemporary::where('numero_encomenda', $this->encomenda)->get();
    ...
    public function cancelarStock()
    {
        ...
        MovimentosDevolucoesClientesTemporary::where('numero_encomenda', $this->encomenda)->delete();
    ...
    {
        $movimentosStockTemporarios = MovimentosDevolucoesClientesTemporary::where('numero_encomenda', $this->encomenda)
        ...
        foreach ($movimentosStockTemporarios as $movimentoStock) {
            MovimentosDevolucoesClientes::create([
    ...
    public function enviar($encomenda)
    {
        ...
        $movimentos = DB::table('movimentos_devolucoes_clientes')
            ->where('numero_encomenda', $encomenda)
            ->get();

        // Verifica se há movimentos para enviar
        if ($movimentos->isEmpty()) {
            return redirect()->route('tenant.devolucoesclientes.devolucoesclientesdetail', $encomenda)
        ...
        DB::table('movimentos_devolucoes_clientes')
            ->where('numero_encomenda', $encomenda)
            ->delete();

        return redirect()->route('tenant.devolucoesclientes', $encomenda)
    ...
    public function render()
    {
        ...
        //$this->devolclientSpecific = $this->encomendaRepository->encomendaMovimentos($this->stringEncomenda,$this->perPage);

        $this->devolclientSpecific = $this->encomendaRepository->detalhesdevolucoesclientes($this->encomenda, $this->perPage);

        return view('tenant.livewire.devolucoesclientes.devolucoesclientesdetail', ["devolucoesclientesdetail" => $this->devolclientSpecific, "idEncomenda" => $this->encomenda]);
